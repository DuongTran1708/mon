...
__all__ = [
	"AICTrafficSafetyCameraS2Multiprocess"
]
...
@CAMERAS.register(name="aic_traffic_safety_camera_solution_2_multiprocess")
class AICTrafficSafetyCameraS2Multiprocess(BaseCamera):
	...
	def __init__(
		...
		Args:
			...
			process_index (int):
				Run process parallel for identifycation, `process_index` shows
				the index of process the camera in
		"""
	...
	def init_data_writer(self, data_writer_cfg: dict):
		...
		data_writer_cfg["dets_full_pkl"] = f'{data_writer_cfg["dets_full_pkl"]}/{self.detector_cfg["folder_out"]}'
	...
	def run_detection(self):
		...
		with torch.no_grad():  # phai them cai nay khong la bi memory leak
			for video_index, video_path in enumerate(tqdm(data_loader, desc=f"Detection process {self.process_index}: ")):
				# Base on the number of process
				if (video_index % int(self.data_cfg['process_num'])) != self.process_index:
					continue
				...
				for images, indexes, _, _ in video_loader:
					...
					for index_b, batch in enumerate(batch_instances):
						...
						for index_in, instance in enumerate(batch):
							...
							bbox_xyxy = self.scaleup_bbox(bbox_xyxy, height_img, width_img, ratio=1.5, padding=60)
	...
	def run_identification_scale(self):
		...
		with torch.no_grad():  # phai them cai nay khong la bi memory leak
			for pickle_index, pickle_path in enumerate(tqdm(data_loader, desc=f"Identification process {self.process_index}: ")):

				# Base on the number of process
				if (pickle_index % int(self.data_cfg['process_num'])) != self.process_index:
					continue

				# DEBUG:
				# if "069" not in pickle_path:
				# 	continue
					...
					for index_b, (crop_dict, batch) in enumerate(zip(pickles, batch_instances)):
						for index_in, instance in enumerate(batch):
							...
							result_dict = {
								...
								'height_img': crop_dict['width_img']
							}
							out_dict.append(result_dict)
				...
				for index_crop, crop_dict in enumerate(tqdm(dets_crop, desc=f"Identifying {basename_noext}: ")):
					...
					for index_b, batch in enumerate(batch_instances):
						for index_in, instance in enumerate(batch):
							...
							# if size of bounding box is very small
							if abs(bbox_xyxy[2] - bbox_xyxy[0]) < 2 or abs(bbox_xyxy[3] - bbox_xyxy[1]) < 2:
								continue
	...
	def run(self):
		...
		# NOTE: run identification, multiprocess
		if self.process["function_identify"]:
